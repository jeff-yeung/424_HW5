...
int main(int argc,char **argv) {
    ...
    for (i = 0; i < iFrames; i++) {
        ...
        // Y PSNR - all frames
        tmp[0] = GetPSNR(pcImgBuf1, pcImgBuf2, iImageArea);
        psnr[0] += tmp[0];
        //U PSNR
        ifs1.read((char *)pcImgBuf1, iImageArea/4);
        ifs2.read((char *)pcImgBuf2, iImageArea/4);
        tmp[1] = GetPSNR(pcImgBuf1, pcImgBuf2, iImageArea/4);
        psnr[1] += tmp[1];
        //U PSNR
        ifs1.read((char *)pcImgBuf1, iImageArea/4);
        ifs2.read((char *)pcImgBuf2, iImageArea/4);
        tmp[2] = GetPSNR(pcImgBuf1, pcImgBuf2, iImageArea/4);
        psnr[2] += tmp[2];
